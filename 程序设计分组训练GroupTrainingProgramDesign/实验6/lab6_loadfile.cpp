...
void loadtoarray(char *path, int filetype) {
    ...
    if(filetype==1)
    {
        ...
        item = (int **) malloc(sizeof (int *)*record_num);
    ...
    qsort(&item, record_num, sizeof(int *), cmparray);
    printf("\nafter sort\n");
    show_array(item, record_num);
    showarray(item, record_num);
    for (int i = 0; i < record_num; ++i) {
        free(item[i]);
    }
...
void loadtostructarray(char *path, int filetype) {
    ...
    printf("before sort\n");
    show_structarray(item, record_num);

    qsort(item, record_num, sizeof (DATAITEM), cmpstructarray);
    printf("\nafter sort\n");
    show_structarray(item, record_num);
    free(item);
}
...
void loadtopointerarray(char *path, int filetype) {
    ...
    printf("before sort\n");
    show_pointerarry(item, record_num) ;
    qsort(item, record_num, sizeof (DATAITEM*), cmppionterarray);
    printf("\nafter sort\n");
    show_pointerarry(item, record_num);
    for (int i = 0; i < record_num; ++i) {
        free(item[i]);
...
void loadtolink(char *path, int filetype) {
    ...
    if (filetype == 1) {
        ...
        fscanf(pf, "%d, %d, %d,\n", &pre->data[0], &pre->data[1], &pre->data[2]);
        head->next = pre;
