...
delimiter //
create procedure restaurant_incomplete_order(in restaurant_name char(15))
begin
    select count(*) from ORDER_COMMENT where restaurant = restaurant_name and isFinished = 0;
end //
delimiter ;

create view restaurant_order_view as
    select ORDER_COMMENT.*, customer.address, customer.phone from order_comment, CUSTOMER
    where order_comment.username = CUSTOMER.username;

-- 创建触发器
DELIMITER //
CREATE TRIGGER update_sales_trigger AFTER UPDATE ON ORDER_COMMENT FOR EACH ROW
BEGIN
...
-- 修改订单状态并触发触发器
UPDATE ORDER_COMMENT SET isFinished = 1, text = '' WHERE orderID = 'your_order_id';
