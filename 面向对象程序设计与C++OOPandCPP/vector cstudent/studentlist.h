...
//    friend void out_put_cstudentlist(CNode *p);
...
class larger_output{
public:
    static bool larger_vector(CNode *a, CNode *b) {
        return a->get_student()->get_number() < b->get_student()->get_number();
    }
    static void out_put_cstudentlist(CNode * p){
             p->get_student()->display(cout);
             cout << "\n";
    }
};


inline bool larger_vector(CNode* a, CNode *b){
    return a->get_student()->get_number() < b->get_student()->get_number();
}
...
public:
    ...
    void add_vector(CStudent *p){
        CNode *tmp = new CNode(p);
        head_vector.push_back(tmp);
        ++count;
    }

    void sort_vector(){
        sort(head_vector.begin(), head_vector.end(), larger_output::larger_vector);
    }

    void show_vector(){
        cout << "id\tname\tmajor\t\tscore\t\tsupervisor" << endl;
        for(int i= 0; i<count ; ++i)
        {
            head_vector[i]->pstu->display(cout);
            cout << "\t"<< endl;
        }
        //        for_each(head_vector.begin(), head_vector.end(), out_put_cstudentlist);
    }
    void show_vector_output(){
        cout << "id\tname\tmajor\t\tscore\t\tsupervisor" << endl ;
        for_each(head_vector.begin(), head_vector.end(), larger_output::out_put_cstudentlist);
    }
    ...
    void show() const
    {
        cout << "id\tname\tmajor\tscore\tref" << endl;
        CNode * tmp = head.next;
        while (NULL != tmp)
        {
